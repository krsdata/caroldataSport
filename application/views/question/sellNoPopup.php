	...
	<div id="tradeBuyContent">
				...
				<input name="__RequestVerificationToken" value="#" type="hidden"><input data-val="true" data-val-number="The field ContractId must be a number." data-val-required="The ContractId field is required." id="ContractId" name="ContractId" value="2844" type="hidden">            <input id="TradeType" name="TradeType" value="1" type="hidden">
					<div class="form-group">
						...
						<div class="validation-error x-long full-width">
							...
							<input  class="form-control full-width quantityBuy valid" data-val="true" data-val-number="The field Quantity must be a number." data-val-regex="Only postive whole numbers are allowed" data-val-regex-pattern="^([1-9][0-9]*)$" data-val-required="Quantity is required" id="Quantity" min="1" max="<?php echo $shares->shares; ?>" name="Quantity" placeholder="Enter Quantity" value="<?php if(!empty($offers)){ echo $offers[0]->shares; } ?>" aria-required="true" aria-invalid="false" aria-describedby="Quantity-error" type="number">
						</div>
					</div>
		...
		<div class="col-xs-12 buttons_bottom">
			<div class="float-right">
				<button type="button" class="btn btn-default" data-dismiss="modal" onclick="close_jbox()" id="cancelModal" data-target="#yes_long">Cancel</button>
				<button type="button" class="btn btn-success" id="submitBuy">Preview</button>
			</div>
		</div>
	</div>
</div>
	...
	<script>
			$(document).ready(function(){
				...
				$("#submitBuy").click(function(){
					...
					$.ajax({
								type: "POST",
								url: "<?php echo base_url("market/submit_deal"); ?>",
								...
								success: function (result) {
									...
									if (result == 'preview') {
										...
										$.nmManual('<?php echo base_url("market/sellPreview"); ?>');
