...
class Sports {
	...
	public function get_options_details($id)
	{
		...
		foreach( $featured_op as $featured_options )
		{
			...
			if($featured_options->optionTypeName == 'Game'){ $opkey = 'g_id';$select = "game_title"; $select_img = "image"; $folder = IMG_GAMES_ADMIN; }elseif($featured_options->optionTypeName == 'Team'){ $opkey = 't_id'; $select = "team_name"; $select_img = "image"; $folder = IMG_TEAMS_ADMIN; }elseif($featured_options->optionTypeName == 'Player'){ $opkey='id'; $select = "player_name"; $select_img = "image"; $folder = IMG_PLAYER_ADMIN; }
			elseif($featured_options->optionTypeName == 'Other'){ $opkey='id'; $select = "option_name"; $select_img = "image"; $folder = IMG_OTHER_ADMIN; }
	...
	{
		//$result = array();
		$where =  array('option_id'=>$option_id,'type'=>$type,'deal_type'=>$deal_type,'status'=>1);
	...
	public function  do_matched_deals($option_id,$type,$deal_type,$share_count,$rate)
	{
		...
		//return $result;
		/* echo "<pre>";
		print_r($matched_shares); echo json_encode($matched_shares);  
		die;
		 */
		//$user_id = $this->CI->session->userdata( 'id' );
		$temp_count = 0;
		if(!empty($matched_shares))
		{
			foreach($matched_shares as $matched_s)
			{
				...
				if($temp_count < 0)
				{
					if($deal_type == 'buy')
					{
						if($matched_s->share_id != 0)
						{
							...
							foreach( $get_shares_to_update as $shares_to_update )
							{
								...
								if( $temp_shares < 0 )
								{
									...
									$bal = $matched_s->rate*$share_count;
									$this->CI->wallet->add_balence( $matched_s->offered_by , $bal );			
								}
								else
								{
							...
							$this->CI->Share_Model->insert_row('ai_shares_log', array(	'log_txn' => $log_txn,
							 ...
							 $ar_insert = array(
								...
								$this->CI->Share_Model->insert_row('ai_confirmed_shares',$ar_insert);
							
							$bal_deduct = $matched_s->rate*$share_count;
							$this->CI->wallet->reduce_balence( $matched_s->offered_by , $bal_deduct );
						...
						else
						{
							...
							$up = $this->CI->Share_Model->update_row('ai_offered_shares',array('id'=>$matched_s->id),array('unmatched_shares'=>$remaining_shares,'matched_shares'=>$matched_total));
							$get_linked_offer = $this->CI->Share_Model->get_row('ai_offered_shares',array('id'=>$matched_s->linked_offer_id),'*'); 
							...
							$up1 = $this->CI->Share_Model->update_row('ai_offered_shares',array('id'=>$matched_s->linked_offer_id),array('unmatched_shares'=>$remaining_shares1,'matched_shares'=>$matched_total1));
							
							$arr_share_insert = array(
														'owner_id' => $get_linked_offer->offered_by,
														'option_id' => $get_linked_offer->option_id,
														'rate' => $get_linked_offer->rate,
														'type' => $get_linked_offer->type,
														'shares' => $share_count,
														//'from_offer_id'=>$matched_s->linked_offer_id,
														//'linked_offer_id'=>$matched_s->linked_offer_id,
														'status'=>1
							...
							$this->CI->Share_Model->insert_row( 'ai_shares_log', array(	
											'log_txn'=>$log_txn,
											'owner_id' => $get_linked_offer->offered_by,
											'option_id' => $get_linked_offer->option_id,
											'deal_type' => 'buy',
											'type' => $get_linked_offer->type,
											'buy_rate' => $get_linked_offer->rate,
											'sell_rate' => 0,
											'profit_loss' => 0 ,
											'shares' =>  $share_count,
											'status' => '1'
							...
							$bal_deduct = $get_linked_offer->rate*$share_count;
							$this->CI->wallet->reduce_balence( $get_linked_offer->offered_by , $bal_deduct );							
							//insert shares of buyer and match person and update also
							
							$arr_share_insert_self = array(
														'owner_id'=>$user_id,
														'option_id'=>$matched_s->option_id,
														'rate'=>$matched_s->rate,
														'type'=>$matched_s->type,
							...
							$ins = $this->CI->Share_Model->insert_row('ai_confirmed_shares',$arr_share_insert_self);
							
							$bal_deduct_self = $matched_s->rate*$share_count;
							$this->CI->wallet->reduce_balence( $user_id , $bal_deduct_self );
					...
					else
					{
						...
						$bal_deduct = $matched_s->rate*$share_count;
						$this->CI->wallet->reduce_balence( $user_id , $bal_deduct );
						...
						//simple sell offer update and shares to be insert in both the profiles
							...
							foreach( $get_shares_to_update as $shares_to_update )
							{
								...
								if( $temp_shares < 0 )
								{
									...
									$bal = $matched_s->rate*$share_count;
									$this->CI->wallet->add_balence( $user_id , $bal );			
								}
								else
								{
									$data_insert_share = array();
				...
				else
				{
					...
					if($deal_type == 'buy')
					{
						if($matched_s->share_id != 0)
						{
							...
							foreach($get_shares_to_update as $shares_to_update)
							{
								$temp_shares = $matched_s->unmatched_shares - $shares_to_update->shares;
								if( $temp_shares < 0 )
								{
									$shares_remain_up = $shares_to_update->shares - $matched_s->unmatched_shares;
									$data_update = array('shares'=>$shares_remain_up);
									$res_up = $this->CI->Share_Model->update_row('ai_confirmed_shares',array('id'=>$shares_to_update->id),$data_update);
									...
									$this->CI->Share_Model->insert_row('ai_shares_log', array(	'log_txn'=>$log_txn,
											...
											'buy_rate' => $shares_to_update->rate,
											'sell_rate' => $matched_s->rate,
											'profit_loss' => ( $matched_s->rate - $shares_to_update->rate ) * $matched_s->unmatched_shares ,
											'shares' =>  $matched_s->unmatched_shares,
											'status' => '1'
										  ) );

									$bal = $matched_s->rate * $matched_s->unmatched_shares;
									$this->CI->wallet->add_balence( $matched_s->offered_by , $bal );
								}
								else
								{
							...
							$bal_deduct = $matched_s->rate * $matched_s->unmatched_shares;
							$this->CI->wallet->reduce_balence( $matched_s->offered_by , $bal_deduct );
							///////////////////////////////////  --- code
						}
						else
						{
							...
							$bal_deduct = $get_linked_offer->rate*$matched_s->unmatched_shares;
							$this->CI->wallet->reduce_balence( $get_linked_offer->offered_by , $bal_deduct );							
							//insert shares of buyer and match person and update also
							...
							$bal_deduct_self = $matched_s->rate*$matched_s->unmatched_shares;
							$this->CI->wallet->reduce_balence( $user_id , $bal_deduct_self );
					...
					else
					{
						...
						$remaining_shares1 = $get_linked_offer->unmatched_shares - $matched_s->unmatched_shares;
						$matched_total1 =  $get_linked_offer->matched_shares + $matched_s->unmatched_shares;
						...
						//simple sell offer update and shares to be insert in both the profiles
							...
							$log_txn = time();
							$shr_temp_count = $matched_s->unmatched_shares;
							
							foreach( $get_shares_to_update as $shares_to_update )
							{
								$temp_shares = $shr_temp_count - $shares_to_update->shares;
								if( $temp_shares < 0 )
								{
									$shares_remain_up = $shares_to_update->shares - $shr_temp_count;
									$data_update = array('shares'=>$shares_remain_up);
									$res_up = $this->CI->Share_Model->update_row('ai_confirmed_shares',array('id'=>$shares_to_update->id),$data_update);
									...
									$this->CI->Share_Model->insert_row('ai_shares_log', array(	'log_txn'=>$log_txn,
											...
											'buy_rate' => $shares_to_update->rate,
											'sell_rate' => $matched_s->rate,
											'profit_loss' => ( $matched_s->rate - $shares_to_update->rate ) * $shr_temp_count ,
											'shares' =>  $shr_temp_count,
											'status' => '1'
										  ) );

									$bal = $matched_s->rate * $shr_temp_count;
									$this->CI->wallet->add_balence( $user_id , $bal );			
								}
								else
								{
									...
									$this->CI->Share_Model->insert_row('ai_shares_log', array(	'log_txn' => $log_txn,
											...
											'buy_rate' => $shares_to_update->rate,
											'sell_rate' => $matched_s->rate,
											'profit_loss' => ($matched_s->rate - $shares_to_update->rate)*$shares_to_update->shares ,
											'shares' =>  $shares_to_update->shares,
											'status' => '1'
										  ) );
										  
									$bal = $matched_s->rate*$shares_to_update->shares;
									$this->CI->wallet->add_balence(  $user_id , $bal );	  
								}
								
								$shr_temp_count = $temp_shares;
							}
				...
				$share_count = $temp_count  ;
				
				if( $temp_count <= 0 )
				{
					$share_count_session['unmatched'] = $temp_count;
					$this->CI->session->set_userdata('deal_result',$share_count_session);
					break;
				}
			}
			$share_count_session['unmatched'] = $temp_count;
			...
			if( $temp_count > 0 )
			{
				...
				{
					$this->create_new_sell_offer($option_id,$rate,$type,'buy',$user_id,$share_count);
				}
			}
		}
		else
		{
			...
			$share_count_session['unmatched'] = $share_count;
			$this->CI->session->set_userdata('deal_result',$share_count_session);
		}
		
		$this->CI->Share_Model->getLatestRateOfOption($option_id);
	}
	...
	public function create_new_sell_offer($option_id,$rate,$type,$deal_type,$user_id,$share_count)
	{
		...
		$shares_overall  =  $this->CI->Share_Model->get_sum('ai_confirmed_shares',array( 'type'=>$type,'option_id'=>$option_id,'status'=>1,'owner_id'=>$user_id ),'shares');	
		
		if( $shares_in_offer === null ){ $shares_offered = 0 ; } else { $shares_offered = $shares_in_offer->unmatched_shares; }
		if( $shares_overall === null ){ $shares_conf = 0 ; } else { $shares_conf = $shares_overall->shares; }
		
		$diff_shares = $shares_conf - $shares_offered;
	...
	function getLatestRate($option_id)
	{
		$result = $this->CI->Share_Model->getLatestRateOfOption($option_id); 
		return $result;
	}
}
